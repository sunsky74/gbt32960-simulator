...
计划完成任务后,服务端模式继续演进,以下功能不在本计划任务范围内(权威状态以 README 与代码为准):

- **扩展命令服务端应答与平台下行下发**:按扩展包声明自动应答私有命令、向在线车辆下发 down 命令(`e2505cf`)
- **平台链路多会话**:0x05 平台登入后同一连接承载多车辆会话、断开逐一注销(见 `internal/servermode/conn.go` 与 `platform_link_test.go`)
- **工作台交互升级**:会话/报文双栏 + 详情区、报文流筛选与跟随、TX 遥测与清空日志、设计规范 V2.0 迁移(`1aed361` 页面激活 → `c39cb00` 工作台重构 → `d07a60c` 设计规范迁移)

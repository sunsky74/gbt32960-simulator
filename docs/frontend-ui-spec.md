...
> 反推自真实代码（2026-09 基准）。技术栈事实：**Vue 3 + Ant Design Vue 4 + 自定义 CSS 变量体系**，
...
> 参数速查版见 `docs/ui-design-reference.md`；本手册为完整规范 + 坏味道清单。
...
| 文本主/次/弱/禁用 | `--text-primary…disabled` | 白 0.88/0.65/0.45/0.35 | 黑同透明度阶梯 | colorText 系 |
...
**规则：组件样式一律引用变量，双主题各验一遍；特殊语义色需提供 light 覆盖**（如加密紫 dark `#9254de` → light `#6424c2`）。

### 1.2 字体与排版

- 家族：`-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'PingFang SC', 'Microsoft YaHei', sans-serif`；数据文本（hex/JSON/VIN/时间）统一 `--font-mono` + `font-variant-numeric: tabular-nums`
- 字号梯度（实测）：**11 / 12 / 13 / 14 / 16 / 22**

| 层级 | 字号×字重 | 实例 |
|---|---|---|
| 页面 Hero 标题 | 22×600 | `.hero-title`（解析页,PacketParserPage:663） |
| 品牌位/大标题 | 16×600 | `.brand`、`.sn-head` 15×600（SettingsPage:450,梯度孤例） |
| 正文/卡片标题 | 14×(400/600) | body、`.cc-title`、`.sb-title` |
| 次要说明 text-muted | 13×400/600 | `.toolbar-label`、`.group-title`、SettingRow `.sr-title` 13×500 |
...
| 微标签 | 2px | `.server-proto`、`.st`、`.sr-badge`(3px,孤例) |
| 胶囊 | 999px | 分割条手柄（本轮新定） |
...
| 二级导航（设置页） | **220px** `.settings-nav`（bg-panel + border subtle 盒子,SettingsPage:374） | |
| 顶栏 | 48px `.topbar`（padding 0 16px,gap 12px,border-bottom subtle） | 客户端/服务端页共用;`bar-sep` 1×16 竖线 + `spacer` 弹性 |
| 主工作区外层 padding | 工作台页 `.server-workbench` **12px 16px**;滚动页 `.page-root` **12px 16px**(gap 12px) | 解析页 `.parser-page`/扩展页 `.ext-page` 复用 page-root 但 ext-page 覆盖为 16px(见 §5) |
...
### 2.3 分割条（ResizableDivider / -Col，`style.css` 分割条段）

| 项 | 客户端基类(上下) | 服务端 tight(上下)/col(左右) |
|---|---|---|
| 判定区 | 9px | **12px**（同时构成面板间 12px 呼吸间距） |
| 视觉 | grip 常显 36×3(radius 2) | 透明容器 + **常显胶囊手柄** tight 32×4 / col 4×32(radius 999) |
| hover/拖拽 | grip→48×3 主色 | 手柄→主色并加长至 40px,条底 `--primary-hover-bg` |
| 提示 | `aria-label`（无原生 tooltip） | 同左 |
| 拖拽锁定 | `body.dragging-ns/ew` 禁选中+锁定光标 | 同左 + `user-select:none` |
...
- 固定宽度参考（内联实测）：档案选择 300 / 搜索框 220 / zone 内嵌搜索 160 / 端口 110 / 心跳 180 / 补发数字 200 / 间隔 130 / 数组项 84 / 报文预览弹窗内 select 200
...
- 布局：flex 两端对齐，左（标题+徽标行 → 描述 12px tertiary，max-width 72%）右（action 插槽，flex-shrink 0，gap 8）
- 行高：`padding: 14px 0`；**分隔线** `border-bottom 1px subtle`（`.row-group` 最后一条隐藏）；内容列 **max-width 880px**（`.sb-content`,padding 6px 24px 28px）
- 禁用态（规划中）：标题降为 secondary
- 二级导航项 `.sn-item`：gap 9px / padding 7px 10px（**非 4px 网格,见 §5**）；active 左侧 2px 主色指示条（与侧栏菜单同构）
...
- **宽度实测**：ConsolePanel 应答 520 / 导出 420；RealTimePanel 报文预览 760（`:footer="null"` 只读 + copyable）；ExtensionsPage 导入 440；**Drawer 仅服务端配置一处：placement right / 360px**
...
## 5. 当前发现的不一致与坏味道（Bad Smells）

| # | 位置 | 问题 | 建议 |
|---|---|---|---|
| 1 | `ExtensionCommands.vue:188` | `.row-label color:#888` 硬编码灰,不随主题;同文件 `.row-head` 与全局 `group-row .row-head` 同名同用途**两套实现** | 改 `var(--text-secondary)`,删除 scoped 重复类直接用全局 |
| 2 | `PacketParserPage.vue:1014-1019` | `#ff7875/#cf1322` 硬编码红(双主题手写第二套);style.css 的 `.bc-issue` 已有主题化同语义样式 | 收敛到 CSS 变量或复用 `.bc-issue` |
| 3 | `ExtensionsPage.vue:834-1059` | 混用严重:834/839 success 色双主题手写、1005 `rgba(255,77,79,.35)!important`、1044-1059 JSON 预览块自成深色主题(`#0f0f0f/#d4d4d4/#7fb3ff/#9ece6a`),**浅色主题下不随主题变化** | JSON 块提 token(`--bg-elevated`+mono)或声明为固定暗色语法块;其余改变量 |
| 4 | 全仓 12 处内联 `style="width:NNNpx"` | 控件宽度约束散落模板(ConnectionCard/RealTimePanel/ExtensionCommands/ServerModePage),与 §3.1 表重复维护 | 收敛为工具类(如 `.w-110`)或组件尺寸档位 |
| 5 | `ExtensionsPage.vue:629` | `.ext-page` 覆盖 `.page-root` padding 为 **16px**(基准 12px 16px)——三处页面级留白三种值(12/16、16、设置页 16+内容 24) | 统一 12px 16px;设置页内容列 24px 可保留为变体但应记录 |
| 6 | `SettingsPage.vue:406` | `.sn-item gap:9px / padding:7px 10px` 脱离 4px 网格;`.sr-badge` 10px 字号低于全局 11px 下限、radius 3px 孤例 | 归位 8px gap / 8px padding / 11px 字号 |
| 7 | `SettingsPage.vue:391/450` | `sn-head 14×600` 与 `sn-item active 15×600` 字号倒挂(父小子大)且 15px 为梯度孤例 | 统一 14 或 15 |
| 8 | `ServerModePage.vue`(下发弹窗) | `modal-hint/extcmd-fields/hex-input` 三类 scoped 重写,与 ConsolePanel 的 `.modal-hint` 重复实现 | `.modal-hint` 提升全局 |
| 9 | `style.css:1325` 等三色行 | 未知橙/加密紫在 dark/light 各写两份 rgba 原值(6 处硬编码),仅 dir-link 有变量化先例 | 提 `--warn-unknown/--encrypted` 语义变量 |

**共性结论**：全局体系（style.css 变量 + zone/card 类）覆盖良好,坏味道集中在 **ExtensionsPage / PacketParserPage / SettingsPage 三个较晚开发的页面**——scoped 样式自带小体系、绕开 token;新增页面应零 scoped 颜色,布局类从全局取。

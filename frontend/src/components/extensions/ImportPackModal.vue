...
<template>
  ...
  <a-modal v-model:open="importOpen" title="导入扩展包" :footer="null" :mask-closable="false" width="640px">
        ...
        <div class="spec-body">
          ...
          <table class="spec-table">
            <tbody>
              <tr>
                <td class="k">id</td>
                <td>包唯一标识。2~64 位小写字母/数字/连字符;同 id 导入覆盖旧包</td>
              </tr>
              <tr>
                <td class="k">label</td>
                <td>显示名,非空</td>
              ...
              <tr>
                <td class="k">baseVersion</td>
                <td>协议基准版本,只能 <code>"2016"</code> 或 <code>"2025"</code></td>
              </tr>
              <tr>
                <td class="k">scope</td>
                <td>
                  应用范围数组:<code>"client"</code>(客户端模拟) / <code>"parser"</code>(报文解析);缺省视为
                  <code>["client"]</code>
                </td>
              </tr>
            </tbody>
          ...
          <p class="spec-p">
            拼在标准 0x02 实时报文尾部的私有 TLV(单元码 1B + 长度 2B + 数据),随实时/补发/周期上报一起发送。
          </p>
          <table class="spec-table">
            <tbody>
              <tr>
                <td class="k">key / title</td>
                <td>组键(同包唯一,禁冒号) / 表单显示标题</td>
              </tr>
              <tr>
                <td class="k">unitCode</td>
                <td>单元码,只允许 <code>0x80~0xFE</code>(128~254),同包唯一;标准码位禁止占用</td>
              </tr>
              <tr>
                <td class="k">enabled</td>
                <td>默认是否启用(false 时表单不勾选、不参与编码)</td>
              </tr>
              <tr>
                <td class="k">multiple / maxRows</td>
                <td>true 时表单多行,<strong>每行编码为一个独立 TLV</strong>;maxRows 上限</td>
              </tr>
              <tr>
                <td class="k">fields</td>
                <td>字段表,至少一项,见下节 DSL</td>
              </tr>
            </tbody>
          </table>
          ...
          <table class="spec-table">
            <tbody>
              <tr>
                <td class="k">u8 / u16 / u32</td>
                <td>无符号整数,1 / 2 / 4 字节</td>
              </tr>
              <tr>
                <td class="k">i8 / i16 / i32</td>
                <td>有符号整数,1 / 2 / 4 字节</td>
              </tr>
              <tr>
                <td class="k">f32</td>
                <td>IEEE 754 浮点,4 字节(不支持 scale/offset)</td>
              </tr>
              <tr>
                <td class="k">bits</td>
                <td>位段开关:<code>bits: [{ "index": 0~31, "label": "…" }]</code>,宽度按最大位号自动取 1/2/4 字节</td>
              </tr>
              <tr>
                <td class="k">bytes</td>
                <td>定长原始字节,<code>length</code> 1~255,表单填 hex(如 1A2B)</td>
              </tr>
              <tr>
                <td class="k">tail</td>
                <td>变长尾部(hex 原样追加),建议放字段表末尾</td>
              </tr>
            </tbody>
          </table>
          <p class="spec-p">
            每个字段:<code>key</code>(组内唯一)、<code>label</code>、<code>type</code>、<code>unit</code>(仅展示)。
            数值变换契约:<strong>物理值 = 线值 × scale + offset</strong>(scale 默认 1、offset 默认 0;scale 须大于 0
            且换算结果须为整数线值)。 例:<code>{ "type": "u16", "scale": 0.1 }</code> 填 12.3 V → 线值 123;温度惯例
            <code>{ "type": "i8", "offset": 40 }</code>。
          </p>
          ...
          <table class="spec-table">
            <tbody>
              <tr>
                <td class="k">key / label</td>
                <td>命令键(同包唯一,禁冒号、禁与标准组键同名) / 显示名</td>
              </tr>
              <tr>
                <td class="k">code</td>
                <td>命令码,上行预留区:2016 为 <code>0x09~0x7F</code>,2025 为 <code>0x0C~0x7F</code></td>
              </tr>
              <tr>
                <td class="k">direction / trigger</td>
                <td><code>"up"</code> / <code>manual</code> · <code>periodic</code> · <code>manual+periodic</code></td>
              </tr>
              <tr>
                <td class="k">body.type</td>
                <td>
                  <code>fields</code> 平铺字段表(单行);<code>realtimeLike</code> 与实时报文同构(6B 时间 + TLV
                  单元,单元结构同第 2 节);<code>remoteSub</code> 0x8A 远控应答模板(配合 fields,仅 2016,详见完整文档)
                </td>
              </tr>
            </tbody>
          </table>
          ...
          <ul class="spec-list">
            ...
            <li>
              key 禁含冒号 <code>:</code>;禁用保留键(vehicle / motor / fuelcell / engine / location / extremum / alarm /
              voltage / temperature 等)
            </li>
            <li>
              导入时做静态校验 + 编码干跑,错误带 JSON 路径(如
              <code>realtime.appendUnits[0].fields[1].scale</code>),照提示修改即可
            </li>
          </ul>
    ...
    <a-alert v-if="jsonError" type="error" show-icon message="导入失败" :description="jsonError" class="import-alert" />

    <div class="modal-footer">
      <a-button @click="importOpen = false">取消</a-button>
      <a-button v-if="importTab === 'file'" type="primary" :loading="importing" @click="importFromFile"
        >选择文件…</a-button
      >
      <a-button v-else-if="importTab === 'json'" type="primary" :loading="importing" @click="importFromJSON"
        >导入</a-button
      >
